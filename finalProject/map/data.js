...
let content = fs.readFileSync('data/html/10.html');
...
$('td[style*="border-bottom:1px solid #e3e3e3;width:350px"]').each(function(i, elem) {
  ...
  // MEETING TYPES
  let typeStr = infoStr.indexOf(' =');
  let typeDef = infoStr.substring(0, typeStr).trim();
  let type = typeDef.split(' Meeting Type').pop().trim();
  // console.log(infoStr)
  console.log(type)
...
//     fs.writeFileSync('data/geo-json/types/10.json', JSON.stringify(meetingGeos));
//     console.log('*** *** *** *** ***');
